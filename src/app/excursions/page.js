...
export default function ExcursionsPage() {
  ...
  const L = locale || 'en'
  const { data: exData, error: exErr, isLoading: exLoading } = useSWR('/api/tours?type=excursion', fetcher)
  const { data: dayData, error: dayErr, isLoading: dayLoading } = useSWR('/api/tours?type=day-tour', fetcher)

  const isLoading = exLoading || dayLoading
  const error = exErr || dayErr
  const items = [
    ...Array.isArray(exData) ? exData : [],
    ...Array.isArray(dayData) ? dayData : [],
  ]

  return (
    <main className="container mx-auto px-4 py-16">
      <h1 className="text-4xl font-extrabold mb-8">{messages.excursions}</h1>
      {isLoading && (
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {items.map((t) => {
          ...
          const imgName = t.slug === 'croisiere-la-goulette' ? 'goulette.jpeg' : ((t.images && t.images[0]) || null)
          const imgSrc = imgName ? `/images/${imgName}` : '/images/placeholder.png'
          return (
